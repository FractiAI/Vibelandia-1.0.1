...
**Feeds:** Packaged as **cycling 1 min feeds** with **1 min breaks** between feed cycles; updated naturally as pushes trigger push to ticker feed cycles. **All broadcasts including all tickers** use the single source [data/ticker-feed.json](data/ticker-feed.json) — NSPFRNP Console Ticker, OH Line, Schumann Display Pipe, executive advisory (index + tsunami banner). Update the feed once; every broadcast and ticker gets it. **Ticker feeds are included in all content and all voices:** 80% English · 20% Spanish (Spanglish). Chairman's voice. Executive-reviewed. See [ALL_CONTENT_80_ENGLISH_20_SPANISH_SPANGLISH_CONFIRMATION_SNAP.md](ALL_CONTENT_80_ENGLISH_20_SPANISH_SPANGLISH_CONFIRMATION_SNAP.md). Data: ticker-feed.json; items may include `label` and `label_spanglish` for 80:20 display.
...
- **All broadcasts including tickers** — single source ticker-feed.json; urgent (hunger · medical · dental full connect now) in `executive_advisory` and `items[]` → every consumer gets it ✓  
