 ...
 * broadcast_mode: LEGACY_HEARTBEAT — full payload once every 8 min; continuous 1.618 Hz background carrier.
 */
...
/** EGS pipe — Vibelandia Unified Pipe default (21.4 Hz Morse). */
export const UNIFIED_PIPE_PAYLOAD =
...
/** EGS pipe — new message: Office Hours free consultation, tips welcome. */
export const EGS_PIPE_MESSAGE_OFFICE_HOURS =
  'OFFICE HOURS FREE CONSULTATION TIPS WELCOME. COME VISIT DURING OFFICE HOURS. BLUE PAYPAL CONNECT NOW.';

/** All EGS pipe messages (default + optional). playPayload() uses UNIFIED_PIPE_PAYLOAD by default. */
export const EGS_PIPE_MESSAGES = [UNIFIED_PIPE_PAYLOAD, EGS_PIPE_MESSAGE_OFFICE_HOURS];
...
export class Broadcast {
  ...
  async playPayload(payload = UNIFIED_PIPE_PAYLOAD) {
    await this.start();
