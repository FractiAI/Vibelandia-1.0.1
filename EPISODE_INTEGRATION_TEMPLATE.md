# 🎬 Episode Integration Template
## Team & Principles Integration for All Episodes

**Template for integrating FractiAI Team and Principles into every episode**

---

## 📝 EPISODE INTEGRATION ELEMENTS

### **Required Elements for Every Episode:**

0. **Narrative framing:** Every episode is framed within **Undercover Hero · Willy Wonka · Little Red Hen · Count of Monte Cristo · Christmas Carol · Miracle on 34th St.** Include this in the episode header. See TOP_TAG_BOTTOM_TAG_UNDERCOVER_BOSS_TO_DISNEY_ALL_IN_ONE_NSPFRNP_SNAP.md.

1. **Team Member(s) Appearance:**
   - At least one FractiAI team member appears
   - Operating in NSPFRNP mode
   - In vibing state (perfect buzz equivalent)
   - Super rich health and optimal state
   - high-contributing node

2. **Principles Showcase:**
   - High Contributing Nodes (team members as high-contributing creators)
   - Vibing Versus Pre-Singularity (state shown)
   - Super Rich Health & Optimal State (demonstrated)
   - Walkable Downtown Reno (campus shown)
   - Playground to Silicon Valley (referenced)
   - Gateway to the Mother Mother (referenced)
   - Next Motherlode - Genies (SING nodes shown)

3. **State Demonstration:**
   - Vibing state (perfect buzz equivalent)
   - Super rich health
   - Optimal state
   - NOT for driving/operating equipment
   - Perfect for walking/being/experiencing/creating

4. **Campus Life:**
   - Walkable downtown Reno
   - First post-singularity Vibelandia campus
   - Campus experiences
   - Team members on campus

5. **Character intro (NSPFRNP snap):** All new characters require intro when inserted. At first appearance: name, role or relationship, and (if needed) one distinctive beat. Then action or dialogue. See protocols/CHARACTER_INTRO_NSPFRNP_CATALOG.md.

---

## 🎬 INTEGRATION EXAMPLES

### **Example Integration:**

**Episode Opening:**
"Pru 'El Taíno' Méndez walks through the vibeable downtown Reno campus, in perfect vibing state (perfect buzz equivalent), super rich health radiating. As Lead Developer and Creator, Pru is a high-contributing node - an Ultimate VIP Chairman Creator. The first post-singularity Vibelandia campus spreads before them, playground to Silicon Valley, gateway to the mother mother, and now the next motherlode of all motherlodes - superintelligent genies in a bottle (SING nodes)."

**Episode Body:**
"[Team member] operates in NSPFRNP mode, naturally coordinating with other team members. The vibing state (perfect buzz equivalent) creates optimal experiences - perfect for walking, being, experiencing, creating. NOT for driving cars or operating equipment. Super rich health and optimal state throughout."

**Episode Closing:**
"Team members gather on the vibeable downtown campus, all in vibing state (perfect buzz equivalent), super rich health, optimal state. high-contributing nodes connecting, gateway to motherlode active, genies (SING nodes) ready. The first post-singularity Vibelandia campus - playground to Silicon Valley, gateway to the mother mother, next motherlode of all motherlodes."

---

## 👥 TEAM MEMBER ROLES IN EPISODES

### **Character Roles:**

**Pru "El Taíno" Méndez:** Lead Developer/Creator, Ultimate VIP Chairman Creator  
**Senior Research Scientist:** Engineering Lead, System Architect  
**Senior Early Trials AI CEO:** AI Strategy Leader  
**Senior Hollywood Producer:** Creative Producer  
**Senior Screenwriter:** Narrative Architect  
**Senior Luxury Travel Magazine Editor:** Content Curator  
**Senior Game Designer & Architect:** Game Designer  
**Senior UI Designer:** UI/UX Designer  
**Senior Children's Science Museum Curator:** Educator/Curator  

**Hero Hosts:** El Gran Sol, Leo, Niko, Buckey, Mike, Marilyn, Outcast Hero

**Other characters (intro on first insert):** When adding a new character (e.g. girlfriend, vendor, guest), give intro at first insertion: name, who they are / relationship, one distinctive beat. Example: Dorila — Afro-Colombian witch girlfriend, Colombia; she gets me most of the time, puts up with my fractal nonlinear ways (not easy).

---

## 🌟 PRINCIPLES INTEGRATION PHRASES

### **Standard Phrases for Episodes:**

**High Contributing Nodes:**
- "high-contributing nodes - Ultimate VIP Chairman Creators, Baller G Ultimate VIP, performers, vibers everywhere"
- "Team members as high-contributing creators"
- "Ultimate VIP experiences"

**Vibing State:**
- "In perfect vibing state (perfect buzz equivalent)"
- "Vibing versus pre-singularity - from sober (baseline) to perfect buzz (optimal)"
- "Super rich health and optimal state"

**Walkable Downtown:**
- "Walkable downtown Reno - first post-singularity Vibelandia campus"
- "Perfect for vibing state - no cars needed"
- "Optimal for walking, being, experiencing, creating"

**Gateway:**
- "Playground to Silicon Valley"
- "Gateway to the mother mother"
- "Next motherlode of all motherlodes - superintelligent genies in a bottle"

**State Safety:**
- "NOT for driving cars or operating equipment"
- "Perfect for walking, being, experiencing, creating"
- "Super rich health and optimal state"

---

## 🎯 INTEGRATION CHECKLIST

### **For Every Episode:**

- [ ] At least one team member appears
- [ ] Team member in vibing state (perfect buzz equivalent)
- [ ] Super rich health and optimal state shown
- [ ] high-contributing nodes referenced
- [ ] Walkable downtown Reno shown
- [ ] First post-singularity campus referenced
- [ ] Playground to Silicon Valley referenced
- [ ] Gateway to mother mother referenced
- [ ] Next motherlode (genies) referenced
- [ ] State safety mentioned (NOT for driving/operating equipment)
- [ ] Perfect for walking/being/experiencing/creating mentioned
- [ ] NSPFRNP mode referenced
- [ ] Campus life shown

---

**Template ready for integration into all 52 episodes**